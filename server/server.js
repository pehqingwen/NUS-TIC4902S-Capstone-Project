...
db.connect(function (err) {
  if (err) {
    ...
    db.query(
      "SELECT product_code FROM order_products WHERE product_code IS NOT NULL",
...
app.post("/submit-form", (req, res) => {
  ...
  db.query(
    ...
    [
      ...
      formData.user_email,
      formData.user_countryCode,
      formData.user_contactNumber,
      formData.user_address1,
      formData.user_address2,
    ...
    (err, result) => {
      if (err) {
        ...
        console.log(
          "Data inserted successfully. Rows affected:",
          result.affectedRows
        );
...
app.post("/checkout", (req, res) => {
  ...
  db.beginTransaction((err) => {
    ...
    const queryUser =
      "INSERT INTO users (user_email, user_firstName, user_lastName, user_address1, user_address2, user_country, user_ZIP, user_countryCode, user_contactNumber) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    db.query(
      ...
      (err, userResult) => {
        ...
        const queryOrder =
          "INSERT INTO orders (user_id, order_totalCost, order_date) VALUES (?, ?, NOW())";

        db.query(
          queryOrder,
          [insertedUserId, order.order_totalCost],
          (err, orderResult) => {
            if (err) {
              return db.rollback(() => {
                throw err;
            ...
            products.forEach((product) => {
              const queryProduct =
                "INSERT INTO order_products (order_id, product_name, product_price, quantity, choice1, choice2, choice3, choice4, choice5, choice6, choice7, choice8, product_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
